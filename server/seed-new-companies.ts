...
export async function seedNewCompanyInternships() {
  ...
  try {
    // Upsert helper: create if missing, otherwise correct the display fields
    // (description format, location, duration) so production stays in sync.
    async function upsertInternship(values: typeof internships.$inferInsert) {
      ...
      if (existing.length === 0) {
        await db.insert(internships).values(values);
    ...
    await upsertInternship({
      ...
      description: `The Kebabsmith
Singapore's boldest healthy frozen food brand on a mission to make wholesome food fun
Company based in Singapore

Ever walked down a supermarket aisle and wondered why some products fly off the shelves while others collect dust? Now you can find out — and get real experience doing it.
...
Fully online. Perfect for anyone who's curious about business, food, or how brands compete.`,
...
If you spend more time analyzing why a Reel went viral than actually watching it — this internship was made for you.
...
Fully online. Ideal for anyone who lives on Instagram, loves Canva, or wants to break into digital marketing.`,
    ...
    await upsertInternship({
      ...
      description: `Chandrani Pearls
India's iconic luxury jewellery house with 40 years of heritage
Company based in Kolkata, India

Did you know there are over a dozen types of pearls — and most people can't tell the difference? That's exactly the problem you'd help solve.
...
Fully online. Great for anyone interested in design, fashion, or brand communication.`,
...
You already know what's trending before it trends. Now imagine getting paid (well, in experience) to prove it.
...
Fully online. Ideal for anyone into fashion, trends, social media analytics, or brand strategy.`,
