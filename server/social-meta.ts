...
// All crawlers that should receive real server-rendered content:
// search engines, AI crawlers, and social-unfurl bots
const ALL_BOTS_UA =
...
async function renderBlog(): Promise<string> {
  // Fetch all posts; seeded posts may have published=false due to DB state
  const posts = await storage.getBlogPosts().catch(() => [] as any[]);
...
export async function socialMetaMiddleware(
  ...
  // Only handle bot/crawler requests — regular browsers get the full SPA
